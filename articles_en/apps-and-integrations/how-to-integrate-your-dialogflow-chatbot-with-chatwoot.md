# How to bring your Dialogflow chatbot to Katalis?Chatbots are valuable for many customer engagement teams. They efficiently handle trivial questions and free human
...
guide, we explain how you can create a bot in Dialogflow and easily integrate it with Katalis in seconds.
...
This completes the basic bot configuration. Let us create a service account and connect it with Katalis.

You can also create additional intents for your specific use cases.
Katalis also supports advanced intents that enables agent handoff, interactive messages, etc.
refer: Scroll down to "Advanced Intents".

Step 3. Create a service account​. To connect this bot with Katalis, you need to create a service account on your
...
create a key so that it can be shared with Katalis. Click on the service account and click on the "Keys" tab. Then,
...
Setting up Dialogflow Integration in Katalis​

Katalis has a native Dialogflow integration. You can connect your bot with Katalis in two quick steps.
...
Once the user requests to talk to the agent, Dialogflow must inform Katalis that an agent can take over the
conversation.
...
Upon triggering an intent with the above payload, Katalis will toggle the status of the conversation to open and hand
it off to an agent.
...
Katalis-Dialogflow integration also supports interactive messages. The following types of interactive messages are
supported.
