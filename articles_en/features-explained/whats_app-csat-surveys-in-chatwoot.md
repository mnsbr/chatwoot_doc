...
WhatsApp. Katalis now supports CSAT surveys via WhatsApp message templates, so surveys can be sent reliably even after
the 24-hour WhatsApp messaging window.

When a conversation is resolved, Katalis can automatically send a short survey to the customer on WhatsApp asking them
...
There is a high chance that CSAT surveys are usually sent after a conversation ends, they must be sent using a WhatsApp
message template approved by WhatsApp.

Katalis takes care of this automatically:
...
You can use CSAT Template surveys if You are using a WhatsApp Cloud or Whastapp Twilio inbox in Katalis
...
Once enabled, Katalis will prepare a WhatsApp template for your survey.
...
1.  Katalis checks if the conversation matches your survey rules
...
Katalis will show a confirmation message explaining that:
