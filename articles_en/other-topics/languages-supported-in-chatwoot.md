# Languages supported in KatalisKatalis natively supports 30+ different languages. This guide will illustrate how to enable a particular language to

work with your dashboard and live chat widget.

Supported languages

Mentioned below are the languages supported in Katalis and the corresponding shortcodes (derived from ISO 639 language
codes).
...
the katalisSettings or by calling the setLocale method. To specify the locale, use the shortcodes provided above. This
will ensure that the widget is displayed in the desired language.

// Pass via window.katalisSettings
...
Katalis does not support language selection at the agent level. Also, note that this language would be used as the
